...
class CheckLogin(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.login_header = MY_LOGIN_INFO["HEADERS"]
    ...
    def test_case03(self):
        """密码错误"""
        from basic_info.url_info import login_url
        data = {'name': 'gbj_use', 'password': '12345678', 'version': 'Europa-3.0.0.19 - 20180428', 'tenant': 'default'}
        res = requests.post(url=login_url, headers=self.login_header, json=data)
        # print(res.status_code, res.text)
        assert res.status_code == 400
        assert json.loads(res.text) == {"err": "the tenant null can not found"}
    ...
    def test_case04(self):
        """租户错误"""
        from basic_info.url_info import login_url
        data = {'name': 'gbj_use', 'password': '12345678', 'version': 'Europa-3.0.0.19 - 20180428', 'tenant': 'defaul'}
        res = requests.post(url=login_url, headers=self.login_header, json=data)
        # print(res.status_code, res.text)
        assert res.status_code == 400
        assert json.loads(res.text) == {"err": "the tenant null can not found"}
    ...
    def test_case06(self):
        """密码为空"""
        from basic_info.url_info import login_url
        data = {'name': 'gbj_use', 'password': '', 'version': 'Europa-3.0.0.19 - 20180428', 'tenant': 'default'}
        res = requests.post(url=login_url, headers=self.login_header, json=data)
        # print(res.status_code, res.text)
        assert res.status_code == 400
        assert json.loads(res.text) == {"err": "the tenant null can not found"}
    ...
    def test_case07(self):
        """租户为空"""
        from basic_info.url_info import login_url
        data = {'name': 'gbj_use', 'password': '123456', 'version': 'Europa-3.0.0.19 - 20180428', 'tenant': ''}
        res = requests.post(url=login_url, headers=self.login_header, json=data)
        # print(res.status_code, res.text)
        assert res.status_code == 400
        assert json.loads(res.text) == {"err": "the tenant null can not found"}
